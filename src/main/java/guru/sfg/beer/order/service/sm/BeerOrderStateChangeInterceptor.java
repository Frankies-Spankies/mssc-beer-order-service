...
public class BeerOrderStateChangeInterceptor extends StateMachineInterceptorAdapter<BeerOrderStatusEnum, BeerOrderEventEnum> {
    ...
    public void preStateChange(State<BeerOrderStatusEnum, BeerOrderEventEnum> state, Message<BeerOrderEventEnum> message,
        ...
        Optional.ofNullable(message).ifPresent(msg -> {
                    Optional.ofNullable((String) msg.getHeaders().getOrDefault(BeerOrderManagerImpl.BEER_ORDER_ID_HEADER, -1L))
                            .ifPresent(beerOrder_id -> {
                                BeerOrder beerOrder = beerOrderRepository.getOne(UUID.fromString(beerOrder_id));
                                beerOrder.setOrderStatus(stateMachine.getState().getId());
